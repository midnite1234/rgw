...
// Добавляем функцию для обновления энергии каждые 3 секунды
setInterval(() => {
    if (energy < 100) {
        energy += 5;
        if (energy > 100) {
            energy = 100; // Ограничиваем энергию до максимального значения (100)
        }
        updateStats();
    }
}, 3000);
...
// Предотвращение масштабирования страницы при касании
...
// Предотвращение копирования текста при долгом нажатии
document.addEventListener('contextmenu', function(event) {
    event.preventDefault();
});

// Предотвращение масштабирования страницы при двойном щелчке
